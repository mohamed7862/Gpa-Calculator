...
// === 5.1 عرض كارت وجدول التحسين بتصميم أنيق ومحاذاة مضبوطة ===
function renderImprovementEngine(uniqueCoursesList, currentCGPA, totalPoints, totalHours) {
    ...
    // أ) حالة الإنذار الأكاديمي (CGPA أقل من 2.00)
    if (currentCGPA < 2.0) {
    ...
    // ب) حالة التراكمي المستقر (تحسين اختياري)
    else {
...
renderSavedSemesters();
updateUI();
handleImportedData();// === 1. الإعدادات والبيانات الأساسية ===
let savedSemesters = JSON.parse(localStorage.getItem('savedSemesters')) || []; 
let courses = []; 
let currentEditingName = null; 
let editingIndex = null; 
